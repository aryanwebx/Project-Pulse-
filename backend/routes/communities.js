...
router.post("/", auth, requireSuperAdmin, async (req, res) => {
  try {
    const { name, subdomain, contactEmail, description = "", settings = {} } = req.body;
    ...
    const community = new Community({
      ...
      settings: {
        ...
        notifications: {
          ...
          newIssues:
            settings.notifications?.newIssues !== undefined
              ? settings.notifications.newIssues
              : true,
          statusUpdates:
            settings.notifications?.statusUpdates !== undefined
  ...
  try {
    ...
    const communitiesWithStats = await Promise.all(
      communities.map(async (community) => {
        ...
        const issueCount = await require("../models/Issue").countDocuments({
