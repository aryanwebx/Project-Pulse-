...
const Issues = () => {
  ...
  const handleClearAllFilters = useCallback(() => {
    if (debounceTimer.current) {
      // Clear pending debounce timer too
      clearTimeout(debounceTimer.current);
    }
    setSearchTermInput(""); // Clear the search input state
    setFilters({
      // Reset the main API filters state
      status: "",
      urgency: "",
      category: "",
      sort: "-createdAt",
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6 pb-10">
      ...
      {successMessage && (
        <div
          className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative transition-opacity duration-300"
          role="alert">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-2xl sm:text-3xl font-bold tracking-[-.035em] text-[#182230]">
            Issues
          </h1>
      ...
      {!loading && issues.length > 0 ? (
        ...
        !loading && ( // Only show if not loading
          <div className="card text-center py-16">
            <div className="text-6xl mb-4" role="img" aria-label="Magnifying glass">
              🔍
            </div>
