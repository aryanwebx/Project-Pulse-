...
import { useAuth } from '../contexts/AuthContext' // To prevent self-editing
...
const ManageUsers = () => {
  ...
  const handleRoleChange = async (userId, newRole) => {
    ...
    try {
      ...
      setMembers(prevMembers =>
        prevMembers.map(m => m._id === userId ? { ...m, role: newRole } : m)
      );
      // Optional: uncomment below to reload from server for guaranteed consistency
      // await loadMembers();
    } catch (err) {
      setError(`Failed to update role: ${err.message}`)
      // Optional: reload to revert optimistic update on error
      // await loadMembers();
    } finally {
  ...
  const handleStatusChange = async (userId, currentStatus) => {
    ...
    try {
      ...
      // Update local state
       setMembers(prevMembers =>
        prevMembers.map(m => m._id === userId ? { ...m, isActive: newStatus } : m)
      );
      // Optional: uncomment below to reload from server
      // await loadMembers();
    } catch (err) {
         ...
         {loading && (
          ...
          <div className="overflow-x-auto relative"> {/* Added relative for overlay */}
            ...
            {members.length === 0 && !loading && (
                <p className="text-center py-4 text-gray-500">No manageable members found in this community.</p>
            )}
          </div>
      </div>
    </div>
