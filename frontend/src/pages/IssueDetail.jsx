...
const IssueDetail = () => {
  ...
  useEffect(() => {
    ...
    const loadIssue = async () => {
      try {
        ...
        // Changed 'error' to 'err' to avoid conflict
  ...
  const handleCategoryChange = async (category) => {
    if (!issue || category === issue.category) return;
    try {
      const updatedIssue = await issueService.updateIssueCategory(issue._id, category);
      setIssue(updatedIssue);
    } catch (err) {
      console.error("Failed to update category:", err.response?.data?.error || err.message);
      setError(`Category update failed: ${err.response?.data?.error || err.message}`);
      throw err;
    }
  };
    ...
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="card bg-red-50 border border-red-200 text-center py-16">
          ...
          <Link to="/app/issues" className="btn-secondary inline-flex items-center space-x-2">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="card text-center py-16">
          ...
          <p className="text-gray-600 mb-6">
            The specific issue you are looking for could not be found, or it may have been deleted.
          </p>
          <Link to="/app/issues" className="btn-primary inline-flex items-center space-x-2">
            <span>Back to Issues List</span>
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="mx-auto max-w-7xl pb-10">
      ...
      <div className="mb-6">
        <Link
          to="/app/issues"
          className="inline-flex items-center space-x-2 text-sm font-semibold text-[#667085] hover:text-[#182230] transition-colors group">
          <svg
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {issue.images && issue.images.length > 0 && <IssueImages images={issue.images} />}
          <IssueDetails
            issue={issue}
            isAdmin={isAdmin}
            categories={
              currentCommunity?.community?.settings?.categories || [
                "Plumbing",
                "Electrical",
                "Security",
                "Cleanliness",
          ...
          {issue.aiAnalysis && (
            <div className="card">
              <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                <span role="img" aria-label="AI brain" className="mr-2 text-xl">
                  🧠
                </span>
                AI Analysis
              </h3>
                    ...
                    <div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2 mt-1">
                        <div
                          ...
                          style={{ width: `${(issue.aiAnalysis.confidence || 0) * 100}%` }}></div>
                      </div>
              ...
              <div className="space-y-2">
                ...
                {currentCommunity.community.contactEmail && (
                  <div className="flex items-center space-x-1 text-sm text-gray-500">
                    ...
                    <a
                      href={`mailto:${currentCommunity.community.contactEmail}`}
